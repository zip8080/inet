...
#define tcpEV (ev.isDisabled()||TCP::testing)?EV_S:EV_S

// macro for more verbose EV << logging (Note: deliberately no parens in macro def)
#define tcpEV2 (ev.isDisabled()||TCP::testing||!TCP::logverbose)?EV_S:EV_S

// testingEV writes log that automated test cases can check (*.test files)
#define testingEV (ev.isDisabled()||!TCP::testing)?EV_S:EV_S
