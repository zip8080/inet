...
simsignal_t XRadio::bitrateSignal = SIMSIGNAL_NULL;
simsignal_t XRadio::radioStateSignal = SIMSIGNAL_NULL;
simsignal_t XRadio::channelNumberSignal = SIMSIGNAL_NULL;
simsignal_t XRadio::lossRateSignal = SIMSIGNAL_NULL;

#define MIN_DISTANCE 0.001 // minimum distance 1 millimeter

Define_Module(XRadio);

XRadio::XRadio() : rs(this->getId())
{
...
void XRadio::initialize(int stage)
{
    ChannelAccess::initialize(stage);

    EV << "Initializing XRadio, stage=" << stage << endl;
    ...
    if (stage == 0)
    {
        ...
        // read parameters
        transmitRange = par("transmitRange").doubleValue();
        transmitAsNoiseRange = par("transmitAsNoiseRange").doubleValue();
        if (transmitRange > transmitAsNoiseRange)
            error("transmitRange cannot be bigger than transmitAsNoiseRange!");
//        if (transmitterPower > (double) (getChannelControlPar("pMax")))
//            error("transmitterPower cannot be bigger than pMax in ChannelControl!");
        rs.setBitrate(par("bitrate").doubleValue());
        rs.setChannelNumber(par("channelNumber").longValue());
        ...
        drawCoverage = (this->hasPar("drawCoverage")) && par("drawCoverage").boolValue();
        updateStringInterval = (this->hasPar("refreshCoverageInterval")) ? par("refreshCoverageInterval").doubleValue() : 0.0;
    }
    else if (stage == 1)
    {
        registerBattery();
...
bool XRadio::processAirFrame(AirFrame *airframe)
{
 ...
 * handleSelfMsg
 */
void XRadio::handleMessage(cMessage *msg)
{
    // handle commands
    ...
    {
        XAirFrame *airframe = check_and_cast<XAirFrame*>(msg);
 ...
 */
void XRadio::bufferMsg(XAirFrame *airframe) //FIXME: add explicit simtime_t atTime arg?
{
...
AirFrame *XRadio::encapsulatePacket(cPacket *frame)
{
    ...
    XAirFrame *airframe = new XAirFrame();
    airframe->setName(frame->getName());
    airframe->setPSend(transmitRange);
    ...
    airframe->setDuration(airframe->getBitLength()/rs.getBitrate());
...
void XRadio::sendUp(AirFrame *airframe)
{
...
void XRadio::sendDown(AirFrame *airframe)
{
 ...
 * considered as noise.
 */
void XRadio::handleUpperMsg(AirFrame *airframe)
{
...
void XRadio::handleCommand(int msgkind, cObject *ctrl)
{
    ...
    {
        error("Transmitter Power change not supported!");
    }
    else
        error("unknown command (msgkind=%d)", msgkind);
}

void XRadio::handleSelfMsg(cMessage *msg)
{
    EV<<"XRadio::handleSelfMsg"<<msg->getKind()<<endl;
    if (msg->getKind()==MK_RECEPTION_COMPLETE)
    {
        EV << "frame is completely received now\n";

        // unbuffer the message
        XAirFrame *airframe = (XAirFrame *)(msg->getContextPointer());
 ...
 */
void XRadio::handleLowerMsgStart(AirFrame* airframe)
{
    ...
    double rcvdPower = airframe->getPSend() / distance;     //FIXME
    ...
    if (airframe->getArrivalTime() == simTime()
            && (airframe->getPSend() > distance)
 ...
 */
void XRadio::handleLowerMsgEnd(AirFrame * airframe)
{
    ...
    if (true && rs.getState() == RadioState::RECV && snrInfo.ptr == NULL)
    {
...
void XRadio::changeChannel(int channel)
{
   ...
   for (RecvBuff::iterator it = recvBuff.begin(); it!=recvBuff.end(); ++it)
   {
    ...
    {
        XAirFrame *airframe = check_and_cast<XAirFrame *> (*it);
        ...
        else if (airframe->getTimestamp() + airframe->getDuration() + propagationDelay > simTime())
        {
            ...
            XAirFrame *frameDup = airframe->dup();
...
void XRadio::setBitrate(double bitrate)
{
...
void XRadio::setRadioState(RadioState::State newState)
{
...
void XRadio::updateSensitivity(double rate)
{
    return;
}

void XRadio::registerBattery()
{
...
void XRadio::updateDisplayString() {
...
void XRadio::enablingInitialization() {
    this->connectReceiver();
    this->connectTransceiver();
}

void XRadio::disablingInitialization() {
    this->disconnectReceiver();
    this->disconnectTransceiver();
}

double XRadio::calcDistFreeSpace()
{
...
void XRadio::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj)
{
    ChannelAccess::receiveSignal(source,signalID, obj);
}

void XRadio::disconnectReceiver()
{
