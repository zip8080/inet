...
int32 SCTPSerializer::serialize(const SCTPMessage *msg, unsigned char *buf, uint32 bufsize)
{
    ...
    for (int32 cc = 0; cc < noChunks; cc++)
    {
        ...
        switch (chunkType)
        {
            case DATA:
            {
                ...
                if (smsg != NULL && smsg->getEncaps()==true)
                {
                }
                else
                {
                    // T.D. 09.02.2010: Only copy data when there is something to copy!
                    const uint32 datalen = dataChunk->getEncapsulatedPacket()->getByteLength();
                    if (smsg != NULL && smsg->getDataArraySize() >= datalen) {
            ...
            case INIT_ACK:
            {
                ...
                // iac->flags = initAckChunk->getFlags();
            ...
            case SACK:
            {
                ...
                struct nr_sack_chunk *nrsac = (struct nr_sack_chunk*) (buf + writtenbytes); // append data to buffer
                ...
                if (sackChunk->getIsNrSack()) {
                    nrsac->nr_of_nr_gaps = htons(sackChunk->getNumNrGaps());
                    nrsac->nr_of_dups = htons(sackChunk->getNumDupTsns());
                } else {
                    sac->nr_of_dups = htons(sackChunk->getNumDupTsns());
                }

                const int32 numgaps = sackChunk->getNumGaps();
                const int32 numnrgaps = sackChunk->getNumNrGaps();
                const int32 numdups = sackChunk->getNumDupTsns();
                struct sack_gap*    gaps;
                struct sack_gap*    nrgaps;
                struct sack_duptsn* dups;
                if (sackChunk->getIsNrSack()) {
                    gaps = (struct sack_gap*)(((unsigned char*)sac) + size_nr_sack_chunk);
                    nrgaps = (struct sack_gap*)(((unsigned char*)sac) + size_nr_sack_chunk + numgaps * sizeof(struct sack_gap));
                    dups = (struct sack_duptsn*)(((unsigned char *)sac) + size_nr_sack_chunk + (numgaps + numnrgaps) * sizeof(struct sack_gap));
                } else {
                    gaps = (struct sack_gap*)(((unsigned char *)sac) + size_sack_chunk);
                    nrgaps = NULL;
                    dups = (struct sack_duptsn*)(((unsigned char *)sac) + size_sack_chunk + numgaps * sizeof(struct sack_gap));
                }
                for (int32 i = 0; i < numgaps; i++) {
                    gaps[i].start = htons(sackChunk->getGapStart(i) - cumtsnack);
                    gaps[i].stop = htons(sackChunk->getGapStop(i) - cumtsnack);
                }
                if (sackChunk->getIsNrSack()) {
                    for (int32 i = 0; i < numnrgaps; i++) {
                        nrgaps[i].start = htons(sackChunk->getNrGapStart(i) - cumtsnack);
                        nrgaps[i].stop = htons(sackChunk->getNrGapStop(i) - cumtsnack);
                    }
                }
                for (int32 i = 0; i < numdups; i++) {
                    dups[i].tsn = htonl(sackChunk->getDupTsns(i));
                }
                break;
            }
            case HEARTBEAT:
            {
                ...
                // hbc->flags = heartbeatChunk->getFlags();
            ...
            case HEARTBEAT_ACK:
            {
                ...
                // hbac->flags  = heartbeatAckChunk->getFlags();
            ...
            case ERRORTYPE:
            {
                ...
                if (errorchunk->getParametersArraySize()>0)
                {
                    ...
                    SCTPParameter* param = (SCTPParameter*)errorchunk->getParameters(0);
                    if (param->getParameterType() == UNSUPPORTED_HMAC)
                    {
                        struct error_cause* cause = (struct error_cause*)(errorchunk + size_chunk);
                        cause->cause_code = htons(UNSUPPORTED_HMAC);
                        SCTPSimpleErrorCauseParameter* errc = check_and_cast<SCTPSimpleErrorCauseParameter*>(param);
                        cause->info[0] = htons(errc->getValue());
                        writtenbytes += ADD_PADDING(6);
                    }
                }
                else
    ...
    if (authstart!=0)
    {
        ...
        HMAC(EVP_sha1(), sharedKey, sizeKeyVector+sizePeerKeyVector, (unsigned char*)ac->data, writtenbytes-authstart, result, &dlen);
    ...
    while (chunkPtr < (bufsize - size_common_header))
    {
        ...
        switch (chunkType)
        {
            ...
            case INIT:
            {
                ...
                chunk->setChunkTypesArraySize(0);
            ...
            case AUTH:
            {
                ...
                HMAC(EVP_sha1(), sharedKey, sizeKeyVector+sizePeerKeyVector, (unsigned char*)sc->data, flen, result, &dlen);
            ...
            case ASCONF:
            {
                ...
                if (cLen > size_asconf_chunk)
                {
                    ...
                    {
                        localAddr = new IPAddress("0.0.0.0");
                        localAddr->set(ntohl(ipv4addr->address));
                    ...
                    while (cLen > size_asconf_chunk+parptr)
                    {
                        ...
                        switch (paramType)
                        {
                            case ADD_IP_ADDRESS:
                            {
                                ...
                                localAddr = new IPAddress("0.0.0.0");
                                localAddr->set(ntohl(v4addr1->address));
                            ...
                            case DELETE_IP_ADDRESS:
                            {
                                ...
                                localAddr = new IPAddress("0.0.0.0");
                                localAddr->set(ntohl(v4addr2->address));
                            ...
                            case SET_PRIMARY_ADDRESS:
                            {
                                ...
                                localAddr = new IPAddress("0.0.0.0");
                                localAddr->set(ntohl(v4addr3->address));
                                priip->setAddressParam(*localAddr);
                                chunk->addAsconfParam(priip);
                                break;
                            }
                            default: printf("TODO: Implement parameter type %d, found in chunk array on %d!\n", paramType, ct);
            ...
            case STREAM_RESET:
            {
                ...
                if ((unsigned int)cLen > sizeof(struct stream_reset_chunk))
                {
                    ...
                    while (cLen > size_stream_reset_chunk+parptr)
                    {
                        ...
                        paramType = ntohs(parameter->type);
                        paramLength = ntohs(parameter->length);
                        switch (paramType)
                        {
                            case OUTGOING_RESET_REQUEST_PARAMETER:
                            {
